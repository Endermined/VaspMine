...
public class MineManager {
	...
	public void loadMines(){
		if(plugin.getConfig().getString("mines") != null){
			...
			plugin.getServer().getConsoleSender().sendMessage("§8[Vasp Mine] - Loading " + listArenas.size() + " mine(s).");
			int loadeadArenas = 0;
			for(String arenaName : listArenas){
				if(plugin.getConfig().getString("mines." + arenaName) != null){
					...
					plugin.getServer().getConsoleSender().sendMessage("§8 *Mine '" + arenaName + "' Successfully loaded.*");
					loadeadArenas++;
				}
			}
			plugin.getServer().getConsoleSender().sendMessage("§8[Vasp Mine] - " + loadeadArenas + " Successfully loaded mine(s).");
		}
